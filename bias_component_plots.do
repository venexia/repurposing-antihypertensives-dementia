...
foreach j in $ht_paper {
	...
	* Perform multiple imputation ----------------------------------------------
	...
	ice instrument exposure male index_age_start cad cbs cvd charlson cons_rate pres_year_* o.never_smoke o.never_drink bmi o.imd2010, saving($data/mi/bias-cohort1-dementia-`j'_imputed_1, replace) m(20) seed(12345) 

 	* Descrbe imputed dataset --------------------------------------------------

	use "$data/mi/bias-cohort1-dementia-`j'_imputed_1.dta", clear
	mi unset
	...
	* Save imputed dataset -----------------------------------------------------
	...
	save "$data/mi/bias-cohort1-dementia-`j'_imputed.dta",replace

	* Use imputed dataset ------------------------------------------------------

	use "$data/mi/bias-cohort1-dementia-`j'_imputed.dta", clear
