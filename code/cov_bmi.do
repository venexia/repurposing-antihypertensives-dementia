...
gen bmi = .
gen bmi_date = .
save "$data/tmp_bmi_only.dta", replace

qui foreach add in `addfiles' {

	* Extract BMI records ------------------------------------------------------

	use patid enttype adid data3 eventdate using "$additional/`add'" ,clear
	keep if enttype==13
	...
	* Save to BMI dataset ------------------------------------------------------
	...
	use "$data/tmp_bmi_only.dta", clear
...
gen weight_date = .
gen index_date = .
save "$data/tmp_bmi_hw.dta", replace
...
use "$data/tmp_bmi_only.dta", clear
append using "$data/tmp_bmi_hw.dta"
gsort patid -bmi_date -weight_date
by patid: egen i = seq()
keep if i==1
drop i
compress
save "$data/cov_bmi.dta", replace
