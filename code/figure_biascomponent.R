...
ggplot2::ggplot(df[df$bin.cont=="continuous",],
                ggplot2::aes(y = coef_scaled,x = forcats::fct_rev(cov.name), color = analysis)) + 
  ggplot2::geom_point(position = ggplot2::position_dodge(width = 0.5)) + 
  ggplot2::geom_errorbar(ggplot2::aes(ymin = ci_lower_scaled, ymax = ci_upper_scaled),
                width = 0, position = ggplot2::position_dodge(width = 0.5)) +
  ggplot2::geom_hline(yintercept=0, linetype = 2) +
  ggplot2::labs(y = "Coefficient and 95% CI", x = "", color = "") +
  ggplot2::scale_y_continuous(lim = c(-13,13),breaks = seq(-12,12,2)) +
  ...
  ggplot2::coord_flip() +
  ggplot2::facet_wrap(exposure~., ncol = 1)

ggplot2::ggsave("output/figure_biascomponent_con.jpeg", 
       height = 297, width = 210, unit = "mm", 
       dpi = 600, scale = 1)

# Generate binary covariate plot ==========================================

ggplot2::ggplot(df[df$bin.cont=="binary",],
                ggplot2::aes(y = exp(coef_scaled),x = forcats::fct_rev(cov.name), color = analysis)) + 
  ggplot2::geom_point(position = ggplot2::position_dodge(width = 0.5)) + 
  ggplot2::geom_errorbar(ggplot2::aes(ymin = exp(ci_lower_scaled), ymax = exp(ci_upper_scaled)),
                         width = 0, position = ggplot2::position_dodge(width = 0.5)) +
  ggplot2::geom_hline(yintercept=1, linetype = 2) +
  ggplot2::scale_y_continuous(trans="log",lim = c(0.7,3.6), breaks = c(seq(0.75,2,0.25),seq(2.5,3.5,0.5))) + 
  ggplot2::labs(y = "Odds ratio and 95% CI", x = "", color = "") +
  ggplot2::theme_minimal() +
  ggplot2::theme(panel.grid.major.y = ggplot2::element_blank(), 
        panel.grid.minor = ggplot2::element_blank(),
        axis.text = ggplot2::element_text(size=8),
...
ggplot2::ggsave("output/figure_biascomponent_bin.jpeg", 
       height = 297, width = 210, unit = "mm", 
       dpi = 600, scale = 1)