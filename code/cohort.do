...
merge m:1 pracid using "$raw/practice_001.dta"
drop _merge
...
quietly foreach list in $dem_cond {
	...
	merge 1:1 patid using "$patlists/patlist_`list'.dta", keep(match master) keepusing(index_date)
...
quietly foreach list in $ht_paper $dem_treat {
	use "$patlists/patlist_`list'.dta", clear
	merge 1:m patid index_date using "$eventlists/eventlist_`list'.dta", keep(match master) keepusing(consid issueseq qty staffid)
	format %td *date
	...
	save "$data/cohortpatlist_`list'.dta", replace
	use "$data/cohort.dta", clear
	merge 1:1 patid using "$data/cohortpatlist_`list'.dta", keep(match master)
	drop _merge
	save "$data/cohort.dta", replace
}
