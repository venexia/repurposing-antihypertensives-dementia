...
* CREATE ETABLE05 - MULTIVARIABLE REGRESSION RESULTS - MULTIPLE IMPUTATION =====

use "$data/etable_imputed.dta", clear
keep exposure outcome sample_size coef stderr pval
export excel using "$output/etables.xlsx", first(var) sheet("eTable05",replace)

* CREATE ETABLE06 - MULTIVARIABLE REGRESSION RESULTS - COMPLETE CASE ===========

use "$data/etable_completecase.dta", clear
keep exposure outcome sample_size coef stderr pval
