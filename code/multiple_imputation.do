...
foreach j in $ht_paper {
	...
	* Perform multiple imputation ----------------------------------------------
	...
	ice outcome exposure instrument male index_age_start cad cbs cvd charlson cons_rate pres_year_* o.smoking o.alcohol bmi o.imd2010, saving($data/data-cohort1-dementia-`j'_imputed_1, replace) m(20) seed(12345) 

	* Descrbe imputed dataset --------------------------------------------------

	use "$data/data-cohort1-dementia-`j'_imputed_1.dta", clear
	mi unset
	...
	* Save imputed dataset -----------------------------------------------------
	...
	save "$data/data-cohort1-dementia-`j'_imputed.dta",replace

	* Use imputed dataset ------------------------------------------------------

	use "$data/data-cohort1-dementia-`j'_imputed.dta", clear
...
keep analysis outcome exposure coef stderr pval ci_lower ci_upper N 
order analysis outcome exposure coef stderr pval ci_lower ci_upper N 
outsheet using "$output/analysis_reg_imputed.csv", comma replace
