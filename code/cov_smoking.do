...
local addfiles : dir "$additional" files "dated_additional_*.dta"
...
gen smoking = .
gen smoking_date = .
save "$data/tmp_adsmoking.dta", replace

qui foreach add in `addfiles' {

	** Extract records ---------------------------------------------------------

	use patid enttype adid data1 eventdate using "$additional/`add'" ,clear
	keep if enttype==4
	...
	** Save to AD dataset ------------------------------------------------------
	...
	use "$data/tmp_adsmoking.dta", clear
...
import delimited using "$covar/smoke_status.csv", clear
gen smoking = .
...
save "$data/tmp_status.dta", replace

** Label smoking RCs -----------------------------------------------------------

use patid medcode eventdate using "$eventlists/eventlist_smoke_status.dta", clear
merge m:1 medcode using "$data/tmp_status.dta"
keep patid smoking eventdate
save "$data/tmp_status.dta", replace
...
use "$data/tmp_adsmoking.dta", clear
append using "$data/tmp_status.dta"
format %td *date
...
compress	
save "$data/cov_smoking.dta", replace
