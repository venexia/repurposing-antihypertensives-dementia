...
foreach x in `adj' {
	...
	merge 1:1 patid using "$data/cov_`x'.dta", keep(match master) keepusing(patid `x')
	local vars = "`vars'" + " `x'"
	keep `vars'
	compress
	save "$data/analysis.dta", replace
...
foreach cm in `cms' {
	comorbid "`cm'"
	use "$data/analysis.dta", clear
	merge 1:1 patid using "$data/cov_`cm'.dta", keep(match master)
...
use "$data/analysis.dta", clear
merge 1:1 patid pracid using "$link/link_ab_patient_imd2010.dta", keep(match master) keepusing(patid pracid imd2010_20)
drop _merge
rename imd2010_20 imd2010
merge 1:1 patid pracid using "$link/link_c_patient_imd2010.dta", keep(match master) keepusing(patid pracid imd2010_20)
drop _merge
...
use "$eventlists/eventlist_anxiety.dta", clear
keep patid index_date
gen anxiety = 1
duplicates drop
save "$data/anxiety_date.dta", replace
...
gen anxiety = 1
duplicates drop
save "$data/anxiety_date_consid.dta", replace

use "$data/analysis.dta", clear
merge 1:1 patid index_date index_consid using "$data/anxiety_date_consid.dta", keep(match master)
drop _merge
...
quietly foreach list in $ht_paper {
	use "$eventlists/eventlist_`list'.dta", clear
	...
	save "$data/initialdose_`list'.dta", replace
	use "$data/analysis.dta", clear
	merge 1:1 patid index_date using "$data/initialdose_`list'.dta", keep(match master)
	drop _merge
	save "$data/analysis.dta", replace
}
