...
foreach j in $ht_paper {
	...
	noi di "$S_TIME : Starting analysis `c' of 7" // length(ht_paper) = 35
	...
	qui {
		...
		* Add instrument -------------------------------------------
		...
		save "$data/analysis/data-cohort`x'-dementia-`j'.dta", replace
		...
		* Perform IV analysis with adjustments ---------------------
		...
		#delimit ;
		regsave using "$data/regresults.dta", 
		pval ci addlabel(analysis, "iv_adj", Fstat, `Fstat', endog, `endog', endogp, `endogp', Hansen, `Hansen', Hansenp, `Hansenp',
		outcome, "dementia", exposure, "`j'", cohort,`x',
		ex_start, `ex_start', ex_drug, `ex_drug', ex_diag, `ex_diag', ex_staff0, `ex_staff0',
		X0Y0, `X0Y0', X1Y0, `X1Y0', X0Y1, `X0Y1', X1Y1, `X1Y1', 
		Z0X0, `Z0X0', Z1X0, `Z1X0', Z2X0, `Z2X0', Z3X0, `Z3X0', Z4X0, `Z4X0', Z5X0, `Z5X0', Z6X0, `Z6X0', Z7X0, `Z7X0',
		...
		* Perform IV analysis with overid test ---------------
		...
		if (`oi1'>1 & `oi2'>1) {
			...
			ivreg2 outcome (exposure=instrument_oi*) pres_year_*, robust ffirst cluster(index_staff) partial(pres_year_*) endog(exposure)
			...
			#delimit ;
			regsave using "$data/regresults.dta", 
			pval ci addlabel(analysis, "iv_oi", Fstat, `Fstat', endog, `endog', endogp, `endogp', Hansen, `Hansen', Hansenp, `Hansenp',
			outcome, "dementia", exposure, "`j'", cohort,`x',
			ex_start, `ex_start', ex_drug, `ex_drug', ex_diag, `ex_diag', ex_staff0, `ex_staff0',
			X0Y0, `X0Y0', X1Y0, `X1Y0', X0Y1, `X0Y1', X1Y1, `X1Y1', 
			Z0X0, `Z0X0', Z1X0, `Z1X0', Z2X0, `Z2X0', Z3X0, `Z3X0', Z4X0, `Z4X0', Z5X0, `Z5X0', Z6X0, `Z6X0', Z7X0, `Z7X0',
		...
		* Calculate instrument-exposure association ----------------
		...
		#delimit ;
		regsave using "$data/regresults.dta", 
		pval ci addlabel(analysis, "first", Fstat, `Fstat', endog, `endog', endogp, `endogp', Hansen, `Hansen', Hansenp, `Hansenp',
		outcome, "dementia", exposure, "`j'", cohort,`x',
		ex_start, `ex_start', ex_drug, `ex_drug', ex_diag, `ex_diag', ex_staff0, `ex_staff0',
		X0Y0, `X0Y0', X1Y0, `X1Y0', X0Y1, `X0Y1', X1Y1, `X1Y1',
		Z0X0, `Z0X0', Z1X0, `Z1X0', Z2X0, `Z2X0', Z3X0, `Z3X0', Z4X0, `Z4X0', Z5X0, `Z5X0', Z6X0, `Z6X0', Z7X0, `Z7X0',
		...
		* Perform provider fixed effects analysis ------------------
		...
		#delimit ;
		regsave using "$data/regresults.dta", 
		pval ci addlabel(analysis, "iv_fe", Fstat, `Fstat', endog, `endog', endogp, `endogp', Hansen, `Hansen', Hansenp, `Hansenp',
		outcome, "dementia", exposure, "`j'", cohort,`x',
		ex_start, `ex_start', ex_drug, `ex_drug', ex_diag, `ex_diag', ex_staff0, `ex_staff0',
		X0Y0, `X0Y0', X1Y0, `X1Y0', X0Y1, `X0Y1', X1Y1, `X1Y1',
		Z0X0, `Z0X0', Z1X0, `Z1X0', Z2X0, `Z2X0', Z3X0, `Z3X0', Z4X0, `Z4X0', Z5X0, `Z5X0', Z6X0, `Z6X0', Z7X0, `Z7X0',
