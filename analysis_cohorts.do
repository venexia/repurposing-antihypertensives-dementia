...
set obs 0
gen analysis = ""
save "$data/regresults_cohorts.dta", replace
...
forval x = 1/5 {
	...
	foreach j in $ht_paper {
		...
		noi di "$S_TIME : Starting analysis `c' of 35" // x*length(ht_paper) = 35
...
outsheet using "$output/analysis_reg_cohorts.csv", comma replace
