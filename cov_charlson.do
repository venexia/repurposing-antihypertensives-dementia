...
local patfiles : dir "$path/covariates/patlists" files "patlist_ci_*.dta"
...
qui foreach file in `patfiles' {
	...
	joinby patid using "$path/covariates/patlists/`file'", unmatched(master)
...
save "$data/covar/charlson.dta", replace
