...
set obs 0
gen analysis = ""
save "$data/regresults_refclasses.dta", replace
...
foreach i in $ht_paper {
	...
	foreach j in $ht_paper {
		...
		if "`i'" != "`j'" {
			...
			noi di "$S_TIME : Starting analysis `c' of 42" // length(ht_paper)*((length(ht_paper)-1) = 42
			...
			qui {
				...
				* Add instrument -----------------------------------------------
				...
				save "$data/analysis/data-cohort`x'-dementia-`i'-`j'-refclasses.dta", replace
				...
				* Perform IV analysis ------------------------------------------
				...
				#delimit ;
				regsave using "$data/regresults_refclasses.dta", 
...
use "$data/regresults_refclasses.dta", clear
...
outsheet using "$output/analysis_reg_refclasses.csv", comma replace
